      ...
      <div className="space-y-3">
        {notes.map((note, i) => (
          <div key={i} className="relative">
            ...
            <button onClick={() => { const u = [...notes]; u[i] = ''; onNotesChange(u); }} className="absolute top-2 right-2 text-[10px] font-bold bg-destructive/10 text-destructive px-2 py-0.5 rounded transition opacity-50 hover:opacity-100">Clear</button>
          </div>
        ))}
      </div>
    </div>
