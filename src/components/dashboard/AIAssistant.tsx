...
const AIAssistant = ({ data, goals }: Props) => {
  ...
  return (
    <div className="bg-card rounded-3xl p-5 shadow-sm border border-border flex items-center gap-4 relative overflow-hidden min-h-[100px]">
      <div className="w-12 h-12 bg-primary text-primary-foreground rounded-full flex items-center justify-center text-2xl shadow-lg shrink-0 animate-bounce">🤖</div>
      <div className="z-10 w-full">
        <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">AI Assistant</p>
        <div className="text-sm md:text-base font-bold text-foreground transition-all duration-500">{insights[currentIndex] || "আপনার ডেটা বিশ্লেষণ করা হচ্ছে..."}</div>
      </div>
    </div>
  );
};
